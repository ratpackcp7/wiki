...
[Back to Blog](https://nextjs.org/blog)
...
[![Jimmy Lai](https://nextjs.org/_next/image?url=%2Fstatic%2Fteam%2Fjimmy.jpg&w=64&q=75&dpl=dpl_CVsvebqJ57dPBS2uJavHac7chDG3)\
\
Jimmy Lai@feedthejim](https://twitter.com/feedthejim)
[![Josh Story](https://nextjs.org/_next/image?url=%2Fstatic%2Fteam%2Fjosh.jpg&w=64&q=75&dpl=dpl_CVsvebqJ57dPBS2uJavHac7chDG3)\
\
Josh Story@joshcstory](https://twitter.com/joshcstory)
[![Sebastian Markbåge](https://nextjs.org/_next/image?url=%2Fstatic%2Fteam%2Fseb.jpg&w=64&q=75&dpl=dpl_CVsvebqJ57dPBS2uJavHac7chDG3)\
\
Sebastian Markbåge@sebmarkbage](https://twitter.com/sebmarkbage)
[![Tim Neutkens](https://nextjs.org/_next/image?url=%2Fstatic%2Fteam%2Ftim.jpg&w=64&q=75&dpl=dpl_CVsvebqJ57dPBS2uJavHac7chDG3)\
\
Tim Neutkens@timneutkens](https://twitter.com/timneutkens)

Ahead of our upcoming [Next.js Conf 2025](https://nextjs.org/conf)
, Next.js 16 is now available.
...
*   [**Cache Components**](https://nextjs.org/blog/next-16#cache-components)
    : New model using Partial Pre-Rendering (PPR) and use cache for instant navigation.
*   [**Next.js Devtools MCP**](https://nextjs.org/blog/next-16#nextjs-devtools-mcp)
    : Model Context Protocol integration for improved debugging and workflow.
*   [**Proxy**](https://nextjs.org/blog/next-16#proxyts-formerly-middlewarets)
    : Middleware replaced by `proxy.ts` to clarify network boundary.
*   [**DX**](https://nextjs.org/blog/next-16#logging-improvements)
    : Improved logging for builds and development requests.

For reminder, those features were available since the previous beta release:

*   [**Turbopack (stable)**](https://nextjs.org/blog/next-16#turbopack-stable)
    : Default bundler for all apps with up to 5-10x faster Fast Refresh, and 2-5x faster builds
*   [**Turbopack File System Caching (beta)**](https://nextjs.org/blog/next-16#turbopack-file-system-caching-beta)
    : Even faster startup and compile times for the largest apps
*   [**React Compiler Support (stable)**](https://nextjs.org/blog/next-16#react-compiler-support-stable)
    : Built-in integration for automatic memoization
*   [**Build Adapters API (alpha)**](https://nextjs.org/blog/next-16#build-adapters-api-alpha)
    : Create custom adapters to modify the build process
*   [**Enhanced Routing**](https://nextjs.org/blog/next-16#enhanced-routing-and-navigation)
    : Optimized navigations and prefetching with layout deduplication and incremental prefetching
*   [**Improved Caching APIs**](https://nextjs.org/blog/next-16#improved-caching-apis)
    : New `updateTag()` and refined `revalidateTag()`
*   [**React 19.2**](https://nextjs.org/blog/next-16#react-192-and-canary-features)
    : View Transitions, `useEffectEvent()`, `<Activity/>`
*   [**Breaking Changes**](https://nextjs.org/blog/next-16#breaking-changes-and-other-updates)
    : Async params, `next/image` defaults, and more
...
For cases where the codemod can't fully migrate your code, please read the [upgrade guide](https://nextjs.org/docs/app/guides/upgrading/version-16)
.

New Features and Improvements[](https://nextjs.org/blog/next-16#new-features-and-improvements)
...
We will be sharing more about Cache Components and how to use them at [Next.js Conf 2025](https://nextjs.org/conf)
 on October 22nd, and we will be sharing more content in our blog and documentation in the coming weeks.
...
Learn more in the documentation [here](https://nextjs.org/docs/app/api-reference/config/next-config-js/cacheComponents)
.
...
### `proxy.ts` (formerly `middleware.ts`)[](https://nextjs.org/blog/next-16#proxyts-formerly-middlewarets)
...
### Logging Improvements[](https://nextjs.org/blog/next-16#logging-improvements)
...
![](https://nextjs.org/_next/image?url=https%3A%2F%2Fh8DxKfmAPhn8O0p3.public.blob.vercel-storage.com%2Fstatic%2Fblog%2Fnext-16%2Fdevelopment-log.jpg&w=1920&q=75)
...
The following features were [previously](https://nextjs.org/blog/next-16-beta)
 announced in the beta release:

### Developer Experience[](https://nextjs.org/blog/next-16#developer-experience)
...
#### Simplified `create-next-app`[](https://nextjs.org/blog/next-16#simplified-create-next-app)
...
Following the [Build Adapters RFC](https://github.com/vercel/next.js/discussions/77740)
, we've worked with the community and deployment platforms to deliver the first alpha version of the Build Adapters API.
...
### Core Features & Architecture[](https://nextjs.org/blog/next-16#core-features--architecture)

#### Enhanced Routing and Navigation[](https://nextjs.org/blog/next-16#enhanced-routing-and-navigation)
...
##### `revalidateTag()` (updated)[](https://nextjs.org/blog/next-16#revalidatetag-updated)

`revalidateTag()` now requires a **[`cacheLife` profile](https://nextjs.org/docs/app/api-reference/functions/cacheLife#reference)
** as the second argument to enable stale-while-revalidate (SWR) behavior:
...
The profile argument accepts built-in `cacheLife` profile names (like `'max'`, `'hours'`, `'days'`) or [custom profiles](https://nextjs.org/docs/app/api-reference/functions/cacheLife#defining-reusable-cache-profiles)
 defined in your `next.config`. You can also pass an inline `{ expire: number }` object. We recommend using `'max'` for most cases, as it enables background revalidation for long-lived content. When users request tagged content, they receive cached data immediately while Next.js revalidates in the background.
...
##### `updateTag()` (new)[](https://nextjs.org/blog/next-16#updatetag-new)
...
##### `refresh()` (new)[](https://nextjs.org/blog/next-16#refresh-new)
...
#### React 19.2 and Canary Features[](https://nextjs.org/blog/next-16#react-192-and-canary-features)

The App Router in Next.js 16 uses the latest React [Canary release](https://react.dev/blog/2023/05/03/react-canaries)
, which includes the newly released React 19.2 features and other features being incrementally stabilized. Highlights include:
...
### Breaking Changes and Other Updates[](https://nextjs.org/blog/next-16#breaking-changes-and-other-updates)

#### Version Requirements[](https://nextjs.org/blog/next-16#version-requirements)
...
#### Removals[](https://nextjs.org/blog/next-16#removals)
...
#### Behavior Changes[](https://nextjs.org/blog/next-16#behavior-changes)
...
#### Deprecations[](https://nextjs.org/blog/next-16#deprecations)
...
#### Additional Improvements[](https://nextjs.org/blog/next-16#additional-improvements)
...
Feedback and Community[](https://nextjs.org/blog/next-16#feedback-and-community)
...
Contributors[](https://nextjs.org/blog/next-16#contributors)

-------------------------------------------------------------

Next.js is the result of the combined work of over 3,000 individual developers. This release was brought to you by:

*   The **Next.js** team: [Andrew](https://github.com/acdlite)
    , [Hendrik](https://github.com/unstubbable)
    , [Janka](https://github.com/lubieowoce)
    , [Jiachi](https://github.com/huozhi)
    , [Jimmy](https://github.com/feedthejim)
    , [Jiwon](https://github.com/devjiwonchoi)
    , [JJ](https://github.com/ijjk)
    , [Josh](https://github.com/gnoff)
    , [Jude](https://github.com/gao_jude)
    , [Sam](https://x.com/samselikoff)
    , [Sebastian](https://github.com/sebmarkbage)
    , [Sebbie](https://github.com/eps1lon)
    , [Wyatt](https://github.com/wyattjoh)
    , and [Zack](https://github.com/ztanner)
    .
*   The **Turbopack** team: [Benjamin](https://github.com/bgw)
    , [Josh](https://github.com/Cy-Tek)
    , [Luke](https://github.com/lukesandberg)
    , [Niklas](https://github.com/mischnic)
    , [Tim](https://github.com/timneutkens)
    , [Tobias](https://github.com/sokra)
    , and [Will](https://github.com/wbinnssmith)
    .
*   The **Next.js Docs** team: [Delba](https://github.com/delbaoliveira)
    , [Rich](https://github.com/molebox)
    , [Ismael](https://github.com/ismaelrumzan)
    , and [Joseph](https://github.com/icyJoseph)
    .
